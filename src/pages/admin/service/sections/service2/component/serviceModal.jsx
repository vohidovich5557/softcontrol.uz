...
export const ServiceCardModal = ({handleClose, open, submit, title, initialValues}) => {
    const {
      register, 
      handleSubmit, 
      reset, 
    ...
    return (
      ...
      <Dialog
        ...
        onClose={handleClose}
      >
        <DialogTitle>{title} Service Card Page</DialogTitle>
        <DialogContent>
              ...
              <form className='w-[100%] flex flex-col items-center gap-[10px]' action='' onSubmit={handleSubmit(submit)}>
                <div className='flex w-[100%] flex-col items-start gap-[10px]'>
                    <label>Image:</label>
                    <input type="file" {...register('name', {required: true})} placeholder='Select Icon for the service' defaultValue={initialValues?.name} className='w-[100%] h-auto outline-none px-[20px] py-[10px] border rounded-[14px]' />
                ...
                <div className='flex flex-col w-[100%] items-start gap-[10px]'>
                    <label>Title:</label>
                    <input type="text" {...register('age', {required: true})} placeholder="enter title for the service" defaultValue={initialValues?.age} className='w-[100%] h-auto outline-none px-[20px] py-[10px] border rounded-[14px]' />
                    {errors.age && <p className='text-red-500 text-[12px]'>{errors.age.message}</p>}
                </div>
                 <div className='flex flex-col w-[100%] items-start gap-[10px]'>
                    <label>Description:</label>
                    <textarea type="text" {...register('idnumber', {required: true})} placeholder="Enter description for the service" defaultValue={initialValues?.idnumber} className='w-[100%] h-[20vh] outline-none px-[20px] py-[10px] border rounded-[14px]'></textarea>
